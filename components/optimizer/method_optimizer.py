...
class MethodOptimization(BaseModel):
    ...
    optimized_method: str = Field(description="Optimized method for evolving the instructions.")
...
class MethodOptimizer(BaseOptimizer):
    def __init__(self, llm, candidate_size: int):
        ...
        self.system_prompt = load_prompt_template("components/optimizer/prompts/system_prompt.prompt")
        self.method_optimization_prompt = load_prompt_template("components/optimizer/prompts/method_optimization_prompt.prompt")
        
    
    def optimize_method(self, method: Optional[str], feedback: str, is_initial: bool = True) -> Optional[List[str]]:
        ...
        prompt = ChatPromptTemplate(
            [
                ...
                ("user", self.method_optimization_prompt)
            ]
        )
        chain = prompt | optimizer
        if is_initial:
            method = self.initial_method_template
        print(f"Optimization has started...")
        try:
    ...
    def optimize(self, method: Optional[str], feedback: str, is_initial: bool = True) -> Optional[List[str]]:
