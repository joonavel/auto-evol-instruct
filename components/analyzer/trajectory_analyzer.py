...
class FailureAnalysis(BaseModel):
    ...
    result: int = Field(
        description="Result of the evolution. If the evolution is passed, please output 1. If the evolution is failed, please output 0."
    )
    reasons: Dict[str, str] = Field(
        description="Reason of why the evolution is failed. Identify cases that failed to evolve, and provide their case ID as key and reasons as value."
    )
    feedback: str = Field(
        description="Feedback to improve the evolution process based on the reasons."
    )
...
class SuccessAnalysis(BaseModel):
    ...
    feedback: str = Field(
        description="Feedback to improve the evolution process. Please provide feedbacks in the context of not only complexity but also diversity and coherence."
    )
...
class TrajectoryAnalysis(BaseModel):
    ...
    response: Union[FailureAnalysis, SuccessAnalysis] = Field(
        description="Response of the evolution trajectory analysis."
    )
...
class TrajectoryAnalyzer(BaseAnalyzer):
    def __init__(self, llm):
        ...
        self.system_prompt = load_prompt_template(
            "components/analyzer/prompts/system_prompt.prompt"
        )
        self.trajectory_analysis_prompt = load_prompt_template(
            "components/analyzer/prompts/trajectory_analysis_prompt.prompt"
        )
    ...
    def analyze_trajectory(self, trajectory: List[List[str]]) -> Tuple[str, str | None]:
        ...
        try:
            if feedback.response.result:
                logging.info("Current method is suitable for the evolution process.")
                return "Passed", feedback.response.feedback
            else:
                logging.info(
                    f"Current method is not suitable:\n{feedback.response.reasons}"
                )
