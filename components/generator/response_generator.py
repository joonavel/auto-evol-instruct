...
class ResponseResult(BaseModel):
    ...
    response: str = Field(
        description="Answer from the given question. If you don't know answer, answer with IDK. Backslash twice in LaTex format."
    )
...
class ResponseGenerator(BaseGenerator):
    def __init__(self, llm, ds_size, batch_size):
        ...
        self.system_prompt = load_prompt_template(
            "components/generator/prompts/system_prompt.prompt"
        )
        self.response_prompt = load_prompt_template(
            "components/generator/prompts/response_prompt.prompt"
        )
    ...
    def generate_response(self, instructions: str) -> str:
        ...
        responses = []
        for k in tqdm(
            range(0, self.ds_size, self.batch_size), desc="Response Generating..."
        ):
