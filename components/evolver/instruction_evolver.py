...
class InitialEvolution(BaseModel):
    ...
    methods: List[str] = Field(
        description="List of methods to make the instruction more complex"
    )
    plan: List[str] = Field(
        description="Comprehensive plan based on the #Methods List# generated in Step 1 to make the #Instruction# more complex. The plan should include several methods from the #Methods List#."
    )
    rewritten_instruction: str = Field(
        description="Rewritten instruction that adds 10 to 20 words into the #Instruction#."
    )
    finally_rewritten_instruction: str = Field(
        description="Finally rewritten instruction after reviewing the #Rewritten Instruction# and identifying any unreasonable parts. Ensure that the #Rewritten Instruction# is only a more complex version of the #Instruction#, make sure that it only adds 10 to 20 words into the #Instruction#. Just provide without any explanation."
    )
...
class InstructionEvolver(BaseEvolver):
    ...
    def handle_parsing_error(
        self, instructions: List[str], batch_output: List[Any]
    ) -> List[str]:
        """
    ...
    def _initial_evolve(self) -> List[List[str]]:
        ...
        for k in tqdm(
            range(0, self.ds_size, self.batch_size), desc="Initial Evolution Loop..."
        ):
            # 마지막 배치 처리
            ...
            try:
                ...
                for _ in tqdm(
                    range(self.loop), desc=f"Batch {k}~{k + self.batch_size} Loop..."
                ):
    ...
    def _iterative_evolve(self, current_method: str) -> List[List[str]]:
        ...
        # 배치 사이즈만큼 데이터를 나누어서 처리
        for k in tqdm(
            range(0, self.ds_size, self.batch_size), desc="Iterative Evolution Loop..."
        ):
            # 마지막 배치 처리
            if k + self.batch_size > self.ds_size:
                instructions = self.dataset[self.instruction_field][k:]
            ...
            try:
                ...
                for _ in tqdm(
                    ...
                    batch_output = chain.batch(batch_input)
                    responses = [
                        output.finally_rewritten_instruction for output in batch_output
                    ]
                    batch_input = [
                        {"steps": current_method, "instruction": response}
    ...
    def evolve(
        self, is_initial: bool, current_method: Optional[str] = None
    ) -> List[List[str]]:
        if is_initial:
