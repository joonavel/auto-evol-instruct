...
class AutoEvolInstruct:
    ...
    def analyze_trajectory(self, trajectory: List[List[str]]) -> Tuple[str, str | None]:
        analyzer = TrajectoryAnalyzer(
            llm=get_deepseek_llm(
                temperature=0.6, top_p=0.95, max_tokens=4096, timeout=120, max_retries=2
            ),
        )
        return analyzer.analyze(trajectory)

    def optimize_method(
        self, method: str, feedback: str, is_initial: bool = True
    ) -> List[str] | None:
    ...
    def validate_method(self, dev_dataset, candidate_methods: List[str]):
        ...
        for idx, method in enumerate(candidate_methods):
            ...
            logging.info(
                f"Dev Instructions has been evolved with {idx}th candidate method."
            )
    ...
    def save_evolution_result(
        ...
        responses: List[str],
        save_path: str = "evolution_result.json",
    ):
    ...
    def run_auto_evol(self, max_step):
        ...
        for step in range(max_step):
            ...
            if step == 0:
                logging.info("Enter Initial Evolution\nCurrent Method : None")
                trajectory = self.evolve_instruction(train_dataset, is_initial=True)
            else:
                logging.info(
                    f"Enter Iterative Evolution\nCurrent Method : {self.current_method}"
                )
                trajectory = self.evolve_instruction(
                    train_dataset, is_initial=False, current_method=self.current_method
                )
            # Trajectory Analysis
            ...
            if result == "Error":
                logging.error(
                    "Trajectory Analysis Result is Error. Please check the trajectory."
                )
                return
            # Method Optimization
            if step == 0:
                candidate_methods = self.optimize_method(
                    None, feedback, is_initial=True
                )
            else:
                candidate_methods = self.optimize_method(
                    self.current_method, feedback, is_initial=False
                )
            if candidate_methods is None:
                logging.error(
                    "Candidate Methods are None. Please check the candidate size."
                )
                return
            # Method Selection
    ...
    def run_evol_instruct(self, method: str, test_run: bool = False):
        ...
        # Saving
        self.save_evolution_result(
            instruction_dataset, child_instructions, responses, self.save_path
        )
        logging.info("Evolution Result has been saved.")
        return
